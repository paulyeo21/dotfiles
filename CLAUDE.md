...
- **`wt done` is explicitly destructive** — Git requires `worktree remove --force` for submodule metadata and cannot reliably report files inside deinitialized submodules. The helper shows advisory status, warns that all worktree/submodule files will be discarded, and requires confirmation; branch deletion still uses safe `-d`.
