...
- **`wt done` + initialized submodules** — Git requires `worktree remove --force` even when clean. The helper verifies the superproject and every initialized submodule first, then uses force only to acknowledge that Git limitation; dirty state is never forced.
